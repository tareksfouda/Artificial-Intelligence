...
public class ConnectFour {
    ...
    private static char getWinnerInRows(char[][] field) {
        ...
        // Otherwise return empty   character, which means nobody win in rows.
        return ' ';
    }
    ...
    private static char getWinnerInDiagonals(char[][] field) {
        // There are 2 kinds of diagonals, check from top left to right bottom here
        for (int column = 0; column < 7; ++column) {
            int count = 0;
        ...
        // check from top right to left bottom
        for (int column = 0; column < 7; ++column) {
            int count = 0;
